...
const MembershipInfo: React.FC<MembershipInfoProps> = ({ memberships, loading, error }) => {
  ...
  if (memberships.length === 0) {
    return <div className="p-4 bg-yellow-100 text-yellow-700 rounded-lg shadow">No active memberships found for this user.</div>;
  }
    ...
    <div className="space-y-4">
      {memberships.map((membership) => (
        <div key={membership.id} className="p-4 bg-white rounded-lg shadow">
          <h2 className="text-xl font-bold mb-2">Membership: {membership.template_id}</h2>
